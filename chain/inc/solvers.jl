...
const _ALGO_DICT_ = Dict("PRAXIS" => :LN_PRAXIS, "SBPLX" => :LN_SBPLX, 
...
function parse_max_iters(max_iters_str::AbstractString)
  ...
  return map(parse, map(strip, split(max_iters_str, ",")));
end
...
const _RHS_ECOORD_ = Float64[N; γz*N; γx*N];
const _RHS_GCOORD_ = Float64[N; γ*N; 0.0];
...
_ω_ecoord_(ϕ::Real, θ::Real) = (ω0 * (cos(θ))^2);
_ω_gcoord_(ϕ::Real, θ::Real) = (ωx * (cos(ϕ))^2 * (sin(θ))^2
                                + ωz * (cos(θ))^2 + 2 * ωxz * cos(θ)*cos(ϕ)*sin(θ));
...
const _LOWER_BOUNDS_ = [0.0, 0.0];
const _UPPER_BOUNDS_ = [2*pi, pi];
...
NLopt_constraint_functions = map(i -> begin
...
const _CONV_SMBLS_ = [:SUCCESS; :STOPVAL_REACHED; :FTOL_REACHED; :XTOL_REACHED];
...
const _NLOPT_SOLVERS_ = [:LN_PRAXIS; :LN_SBPLX; :GN_ISRES; :LD_SLSQP;
                         :LD_LBFGS; :AUGLAG];
const _NLOPT_REQ_BOUNDS_ = [:GN_ISRES];
const _NLOPT_REQ_CONSTRAINTS_ = [:GN_ISRES; :LD_SLSQP; :AUGLAG];
const _NLOPT_DEFAULT_LOWER_BOUNDS_ = [-N; -100.0; -100.0];
const _NLOPT_DEFAULT_UPPER_BOUNDS_ = [100.0*N; 100.0; 100.0];
...
function init_solver(algo::Symbol, x0s::Vector, reltol::Real, abstol::Real,
  ...
  _NUMEVALS_ = 0;
  _ITERS_PER_PRINT_ = convert(Int, round(print_every_percent_compl * max_iters));
  ...
  if algo in _NLOPT_SOLVERS_
    ...
    if algo in _NLOPT_REQ_CONSTRAINTS_
      for i=1:3
        equality_constraint!(opt, NLopt_constraint_functions[i]); 
      end
    end
    if algo == :AUGLAG
