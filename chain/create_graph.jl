...
open(outfile, "w") do w
  for x in xs
    println("x = $x");
    if verbose
      command = `julia exact_kuhn_grun.jl --Ex $Ex --Ez $Ez --dk $dk --kT $kT -N $N --reltol-int $reltol --maxevals-int $maxevals --reltol-opt $reltol_opt --abstol-opt $abstol_opt --maxevals-opt $maxevals_opt -g $x --verbose`;
    else
      command = `julia exact_kuhn_grun.jl --Ex $Ex --Ez $Ez --dk $dk --kT $kT -N $N --reltol-int $reltol --maxevals-int $maxevals --reltol-opt $reltol_opt --abstol-opt $abstol_opt --maxevals-opt $maxevals_opt -g $x`;
    end
